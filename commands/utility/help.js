	...
	let { prefix } = require('../../config.json')
		...
		if (!args[0]) {
		  ...
		  let embed = new MessageEmbed()
			...
			.setDescription(
			  `Use \`${prefix}help\` followed by a command name to get more additional information on a command. For example: \`${prefix}help ping\`, to invite me: \`https://dsc.lol/samari\``
			)
			.setFooter(
			  `Requested by ${message.author.tag} - Samari`,
			  message.author.displayAvatarURL({ dynamic: true })
			)
			.setThumbnail('https://cdn.discordapp.com/attachments/829720880283254819/830372073426911245/art-5865542_1920.jpg')
			.setTimestamp()
			.setColor(roleColor);
