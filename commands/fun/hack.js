...
function makePasswd() {
  var passwd = '';
  var chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#_£%^&*()-+=`~|\ /m:;<>{}[]?.,';
  for (i=1;i<8;i++) {
...
function makeEmail() {
    ...
    var strEmail = ['@gmail.com', '@outlook.com', '@hotmail.com', '@icloud.com', '@yahoo.com'];
    var strTmp = "";
    for (var i = 0; i < 10; i++) {
...
let ipResult = (Math.floor(Math.random() * 255) + 1)+"."+(Math.floor(Math.random() * 255))+"."+(Math.floor(Math.random() * 255))+"."+(Math.floor(Math.random() * 255));
...
message.channel.send(`Hacking ${user}`).then(msg => {
                    ...
                    setTimeout(function() {
                        msg.edit(`**Email: **` + emailResult + "\n**Password:** " + passResult).then(msg => {
                    setTimeout(function() {
